...
*******************************************/
 ...
 int main( int argc, char *argv[])
 {
	...
	char  strBufr[1024] = "12123456789012";
...
***********************/
	...
	//	printf("111111111 strBufr [%s] ==== \n", strBufr);	
		if( FD_ISSET(s, &read_fds))
		{	
	//	printf("===== strBufr [%s] ==== \n", strBufr);	
		//	memcpy( bufall, strBufr, sizeof(bufall) );
		    
			for( i = 1; i < 100; i++)
			{
				memset(bufall, 0x00, sizeof(chBuffer));
			    ...
			    strncat(bufall, "71",       2);
				strncat(bufall, chPhone,   11);
				strncat(bufall, chName,    20);
				strncat(bufall, chCardNum, 20);
				strncat(bufall, chCardInc, 20);
				
			printf("===== buf [%s] ==== \n", bufall);	
				if( send( s, bufall, 73, 0) < 0 )
